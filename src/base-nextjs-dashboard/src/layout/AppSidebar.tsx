...
const othersItems: NavItem[] = [
  ...
  },
  {
    icon: <ListIcon />,
    name: "Custom Fields",
    key: 'sidebar.customFieldDefinitions',
...
const AppSidebar: React.FC = () => {
  ...
  const filteredOthersItems = othersItems.filter(item => {
    if (user?.role === USER_ROLE.MANAGER.value && item.key === 'sidebar.users') {
      return false;
    }
    return true;
  });
  ...
  useEffect(() => {
    ...
    if (!submenuMatched) {
      // eslint-disable-next-line react-hooks/set-state-in-effect -- close the submenu when the browser's current pathname matches none (external navigation state)
      setOpenSubmenu(null);
    }
  }, [pathname, isActive, filteredOthersItems]);
